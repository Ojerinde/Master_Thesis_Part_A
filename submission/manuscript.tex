...
This paper evaluates fourteen detectors under attacks that are radio signals rather than
...
front end, and tracked by the FGI-GSRx software receiver \citep{fgigsrx}. The observables
...
the detector's inputs. The adversary acts on transmitter settings, listed in
...
\begin{itemize}
  \item An evaluation methodology for GNSS spoofing detectors in which every attack is
  synthesised as radio samples, summed with authentic recordings and tracked by a software
  receiver, so that the detector inputs are produced by receiver processing of a
  transmittable waveform. The generator is validated by reproducing a recorded attack whose
  construction is published.

  \item A library of 209 transmitter settings spanning counterfeit power, code pull-off
  rate and carrier alignment, of which 126 satisfy the published conditions for receiver
  takeover. The library expresses the adversary's choice set as data and separates
  transmissions that capture a receiver from those that do not.

  \item A comparative evaluation of fourteen detectors, eight classical and six deep,
  against that library at a common operating point, reporting for each detector the number
  of transmitted attacks it fails to flag together with the false alarm rate it incurs on
  authentic data.

  \item A navigation domain measurement establishing that a missed attack reaches the
  position and timing solution, obtained by propagating one such attack through the
  navigation stage and comparing the induced clock drift against the rate the transmitter
  was configured to impose.
\end{itemize}
...
that the reported figures are substantially determined by the evaluation corpora rather
than by architectural choices.
...
different distribution than the training attacks but are still drawn rather than chosen.
...
Table~\ref{tab:features} lists the nine observables, exported once per epoch for each
tracked satellite.
...
\caption{Time history of the measurements for one satellite, PRN 23, across the onset of a recorded attack. The measurements move together as the counterfeit takes over the tracking loops, which is the signature a detector reads.}
...
threat model is stated in terms of a transmitter rather than in terms of a displacement in
a feature space.
...
rather than two competing peaks. A misaligned counterfeit does not transfer the tracking
loops; it perturbs the victim's observables without displacing its position or time
solution, and therefore constitutes interference rather than spoofing. The counterfeit must
...
gain control and in the reported $C/N_0$.

Only once both conditions hold does the adversary begin to displace the code. These
...
Clean detection performance does not predict resistance to transmitted attack, and at the
top of the ranking the two orderings are close to inverted. The random forest attains both
the highest clean $F_1$ score and the lowest false alarm rate of the fourteen detectors,
and it is nonetheless evaded by 43.7 percent of the transmitted takeover attacks. This is
not a marginal effect confined to one model: the four best detectors on clean data occupy
four of the five worst positions under transmitted attack. Because all fourteen detectors
are read at the same recall floor, the inversion cannot be attributed to differences in
threshold placement, and because the attacks were selected without reference to any
detector, it cannot be attributed to adversarial optimisation against a particular model.
...
A missed detection is consequential rather than benign. The attack propagated through the
...
exhibit is a property of the models rather than of either evaluation. For the support
...
are the inputs to such a scheme rather than a substitute for it.
...
what an unrestricted adversary could achieve.
...
reported position rather than the clock would apply differing offsets across satellites,
...
advance rather than estimated after the fact \citep{cohen2019certified}. Both lie outside
...
Clean detection performance was found to be a poor predictor of resistance, and at the top
of the ranking the two orderings were close to inverted. The random forest attained the
highest clean $F_1$ score at 0.833 and the lowest false alarm rate at 0.337, and failed to
flag 55 of the 126 attacks, flagging 0.1 percent of the spoofed samples of one of them. The
