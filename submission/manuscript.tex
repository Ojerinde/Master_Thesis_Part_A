...
power. The published procedure has three stages \citep{psiaki2016gnss,kerns2014uav}. The counterfeit is first aligned with the authentic
signal in code phase, carrier phase and Doppler frequency, so that the victim's correlator
sees a single undistorted peak. Its power is then raised slightly above the authentic
power, typically by a few decibels. Only then is it dragged away, carrying the tracking
loops with it and moving the reported position or time. A counterfeit that omits the
...
sample level and summed with authentic recorded samples from the Texas Spoofing Test Battery
\citep{humphreys2012texbat,humphreys2016texbatds78}. The sum is passed through a model of the
receiver front end and tracked by the FGI-GSRx software receiver \citep{fgigsrx}, published by the
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig01_pipeline_plain.png}
...
recall, and it also pays the lowest false alarm rate. It nonetheless fails to flag 55 of the
126 transmitted attacks that capture the receiver, flagging as few as 0.1 percent of the spoofed samples of one of
...
The principal contribution is the evaluation methodology, because the findings that follow
hold only if the detector inputs are produced by receiver processing of a signal an
adversary could transmit. The remaining three rest on it: the second supplies the
adversary's choice set, and the third and fourth measure what existing detectors do against
that choice set and what a failure costs.

\begin{enumerate}
  \item \textbf{An evaluation methodology in which every attack is a transmitted
  waveform.} Each attack is synthesised as radio samples, summed with authentic recordings
  and tracked by a software receiver, so that the detector inputs are produced by receiver
  processing of a transmittable waveform and realizability is settled by the construction
  of the experiment. The generator is validated by reproducing a recorded attack whose
  construction is published.

  \item \textbf{A library of transmitted attacks that expresses the adversary's choice
  set as data.} The sweep covers 209 transmitter settings in counterfeit power, code
  pull-off rate and carrier alignment, of which 126 satisfy the published conditions for
  receiver takeover. Separating the two is what allows a detector to be scored only on
  transmissions that would have captured a receiver.

  \item \textbf{Evidence that clean detection performance does not predict resistance
  to transmitted attacks.} Fourteen detectors, eight classical and six deep, are measured
  against that library at a common operating point. The four detectors ranked highest on
  clean data are evaded significantly more often than the remaining ten, and the detectors
  evading no attack pay false alarm rates of 46 to 74 percent on authentic data. Every
  count carries an exact binomial interval, and the contrast is reported as a difference
  between the top of the ranking and the rest, which the data support, and not as a
  monotone trend across all fourteen, which they do not.

  \item \textbf{A navigation domain measurement establishing that a missed attack
  reaches the position and timing solution.} One evaded attack is propagated through the
  navigation stage, and the clock drift it induces is compared against the rate the
  transmitter was configured to impose. Detection failure is therefore shown to be a
  navigation failure and not only a classifier error.
\end{enumerate}
...
own tracking, so the alignment condition holds by construction. What is measured is how a
detector behaves once capture has been achieved, not how hard it was to achieve. These
...
counts given are the fewest such an adversary would produce: relaxing any restriction
enlarges what the adversary may transmit and can only increase them. The fourth restriction additionally fixes the observable consequence measured
...
What the data support is a difference between two groups of detectors, and not a steady
trend running across all fourteen. Section~\ref{sec:res_stats} reports a Spearman correlation of $+0.41$
...
each restriction reduces its capability, so the failure rates reported are the fewest an
unrestricted adversary would achieve and not an estimate of what it would achieve. The power condition illustrates the point.
...
the signals such a detector would have to be evaluated against, including the settings
that hold received power constant through Equation~\ref{eq:neutral}. The second is
...
satellite coverage and a stationary victim. Each restriction reduces its capability, so an
adversary holding none of them would evade at least as often as reported here.
