...
to transmitted attacks. The detectors that score best on clean data are among those most
often evaded, and the detectors that are never evaded reach that position only by raising
false alarms on authentic data to a level a receiver could not accept. We also follow one
missed attack through the navigation stage and show that it reaches the timing solution,
so detection failure is not confined to the classifier. The manuscript reports the
supporting counts, intervals and rates in full. This speaks
